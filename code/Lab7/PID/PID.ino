  ...
  switch (cmd_type) {
      ...
      case SET_LINEAR_PID:
        ...
        if (!robot_cmd.get_next_value(minPWM))
          return;
        if (!robot_cmd.get_next_value(maxPWM))
          return;
...
void save_ToF() {
    if (distanceSensorS.checkForDataReady() && distanceSensorF.checkForDataReady()){
      ...
      Serial.println(fDistance[indexToF]);
...
float calculatePID(int currentDistance) {
  ...
  float dt = timePID[indexPID]-timePID[indexPID-1];
  ...
  float Dout = Kd * (error - previousError)/dt; // Derivative term
